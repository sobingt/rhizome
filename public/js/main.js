  ...
  $.getJSON('dev/demo.json', function(data) {
    ...
    $.each(data.options, function(i, option) {
      ...
      if (option.content.length > 120) {
        optionHTML += option.content.substring(0,120) + '... <a href=\"#\" data-reveal-id=\"text-' + option.id + '\">more</a>';
      } else {
      ...
      // add text modal
      var textHTML = '<div id=\"text-' +
        option.id + '\" class=\"reveal-modal large\"><h4>' +
        option.title + '</h4><dl class=\"sub-nav\"><dd class=\"active\"><dd class=\"active\"><a href=\"#\">Text</a></dd><dd><a href=\"#\" data-reveal-id=\"discussion-' +
        option.id + '\">Discussion (' +
        option.argument_count + ')</a></dd></dl><p>' +
        option.content + '</p><a class=\"close-reveal-modal\">&#215;</a></div>';
      $(textHTML).appendTo('body');

      function addArgument(argument) {
        argumentsHTML += '<li><div class=\"argument\"><div class=\"buttons\"><a href=\"#\" class=\"tiny button radius success promote-button\">Promote</a> <a href=\"#\" class=\"tiny button radius promote-button\">Reply</a></div><span class=\"id\">' +
          argument.id + '</span><span class=\"timestamp\">' +
          argument.timestamp + '</span><p>' +
          argument.text + '</p></div><ul>';
      ...
      var discussionHTML = '<div id=\"discussion-' +
        option.id + '\" class=\"reveal-modal large\"><h4>' +
        option.title + '</h4><dl class=\"sub-nav\"><dd class=\"active\"><dd><a href=\"#\" data-reveal-id=\"text-' +
        option.id + '\">Text</a></dd><dd class=\"active\"><a href=\"#\">Discussion (' +
